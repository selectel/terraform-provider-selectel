...
func TestAccGlobalRouterZoneV1DataSourceBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccGlobalRouterZoneV1DataSourceBasic(dataGRZoneName, testZoneName),
...
func TestAccGlobalRouterZoneV1DataSourceWithService(t *testing.T) {
	dataGRZoneName := "gr_zone_1"
	testZoneName := "ru-1"
	testService := "vpc"
	dataSourceZoneName := fmt.Sprintf("data.selectel_global_router_zone_v1.%[1]s", dataGRZoneName)
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccGlobalRouterZoneV1DataSourceWithService(dataGRZoneName, testZoneName, testService),
				Check: resource.ComposeTestCheckFunc(
					testAccZoneV1Exists(dataSourceZoneName),
					resource.TestCheckResourceAttr(dataSourceZoneName, "name", testZoneName),
					// computed opts
...
func testAccGlobalRouterZoneV1DataSourceBasic(resourceName, zoneName string) string {
	return fmt.Sprintf(`
...
func testAccGlobalRouterZoneV1DataSourceWithService(resourceName, zoneName string, service string) string {
	return fmt.Sprintf(`
data "selectel_global_router_zone_v1" %[1]q {
  name=%[2]q
  service=%[3]q
