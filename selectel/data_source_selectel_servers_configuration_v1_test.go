		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVPCV2ProjectExists("selectel_vpc_project_v2.project_tf_acc_test_1", &project),
					testAccServersConfigurationV1ServerChipExists("data.selectel_servers_configuration_v1.serverchip_configuration_tf_acc_test_1", configurationName),
					resource.TestCheckResourceAttr("data.selectel_servers_configuration_v1.serverchip_configuration_tf_acc_test_1", "configurations.0.name", configurationName),
				),
			},
		},
	})
...
func testAccServersConfigurationV1ServerChipExists(n string, serverName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]
		if !ok {
			return fmt.Errorf("not found: %s", n)
