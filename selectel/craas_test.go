...
func newCRaaSTestClientV2(rs *terraform.ResourceState, testAccProvider *schema.Provider) (*clientv2.ServiceClient, error) {
	config := testAccProvider.Meta().(*Config)
	...
	if err != nil {
		return nil, fmt.Errorf("can't get client for craas V2 acc tests: %w", err)
	}
